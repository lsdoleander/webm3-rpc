	...
	import { zergpool, zpool } from './fetchpool.js' 
	...
	export default function init(port=PORT) {
		let app = express();
		app.use(cors());
		...
		for (let idx in mappings) {
			mappings[idx].socket = socket(app, mappings[idx])
		}
		...
		app.get("/zergpool", (request, response)=>{
			zergpool().then(data=>{
				response.send(data);
			})
		})

		app.get("/zpool", (request, response)=>{
			zpool().then(data=>{
				response.send(data);
			})
		})
